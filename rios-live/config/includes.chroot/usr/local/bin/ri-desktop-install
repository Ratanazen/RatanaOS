#!/usr/bin/env bash
# ==============================================================================
# RiOS Master Desktop Auto-Installer & Environment Provisioner
...
LOG_SYS="/var/log/rios/install.log"
LOG_USER="${TARGET_HOME}/.local/state/rios/install.log"
...
print_banner() {
    echo -e "\e[34m╭──────────────────────────────────────╮\e[0m"
    echo -e "\e[34m│\e[0m           \e[1;37mRIOS DESKTOP\e[0m           \e[34m│\e[0m"
...
install_themes() {
    ...
    mkdir -p /usr/share/rios/themes /etc/rios
    if [ -d "${ROOT_DIR}/themes" ]; then
        cp -rf "${ROOT_DIR}"/themes/* /usr/share/rios/themes/ 2>/dev/null || true
    elif [ -d "/usr/share/rios/themes" ]; then
        :
    fi
    echo -e "\e[1;32m✓\e[0m"
    log "INFO" "RiOS themes installed"
}

install_configs() {
    USER_CONFIG="${TARGET_HOME}/.config"
    BACKUP_DIR="${USER_CONFIG}/ri/backups/$(date +%Y%m%d_%H%M%S)"
    mkdir -p "${BACKUP_DIR}" "${USER_CONFIG}/ri"
...
configure_services() {
    ...
    if [ "$DRY_RUN" -eq 0 ]; then
        ...
        if [ -d "${ROOT_DIR}/rios-live/config/includes.chroot/usr/share/wayland-sessions" ]; then
            cp -rf "${ROOT_DIR}"/rios-live/config/includes.chroot/usr/share/wayland-sessions/* /usr/share/wayland-sessions/ 2>/dev/null || true
        fi
    fi
    echo -e "\e[1;32m✓\e[0m"
...
echo -e "\n\e[1;32mRiOS Desktop Ready.\e[0m"
