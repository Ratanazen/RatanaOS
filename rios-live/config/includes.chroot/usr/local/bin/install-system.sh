#!/usr/bin/env bash
# ==============================================================================
# RiOS Installer: Base System Deployment & Live-Boot Removal Module
# ==============================================================================
set -euo pipefail
...
install_system_files() {
    ...
    for path in \
        ...
        /workspace/rios-live/binary/live/filesystem.squashfs; do
        if [ -f "$path" ]; then
            squashfs_path="$path"
            break
        fi
    ...
    echo "==> Deploying RiOS system files to $target_mount..."
...
purge_live_components() {
    ...
    chroot "$target_mount" bash -c "
        ...
        rm -f /etc/sudoers.d/live /etc/sudoers.d/99_ri_live 2>/dev/null || true
