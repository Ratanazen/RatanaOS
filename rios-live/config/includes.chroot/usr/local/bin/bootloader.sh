#!/usr/bin/env bash
# ==============================================================================
# RiOS Installer: GRUB Bootloader Installation Module (UEFI / BIOS)
# ==============================================================================
set -euo pipefail
...
install_bootloader() {
    ...
    # Configure /etc/default/grub for RiOS
...
GEOF
    ...
    if [ "$mode" = "uefi" ]; then
        ...
        if [ -f "${target_mount}/boot/efi/EFI/RiOS/grubx64.efi" ]; then
            cp -f "${target_mount}/boot/efi/EFI/RiOS/grubx64.efi" "${target_mount}/boot/efi/EFI/BOOT/BOOTX64.EFI"
        fi
    else
