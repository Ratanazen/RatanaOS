# 🔍 RiOS Deviations from Stock Debian Behavior

This document tracks all intentional architectural and configuration differences between standard Debian GNU/Linux 12 ("Bookworm") and RiOS.
...
| Component | Stock Debian Bookworm Behavior | RiOS Modified Behavior | Motivation / Rationale |
| :--- | :--- | :--- | :--- |
| **System Identity** | `NAME="Debian GNU/Linux"`, `ID=debian` | `NAME="RiOS"`, `ID=rios`, `ID_LIKE=debian` | Distro branding while maintaining 100% tooling compatibility |
...
| **Default Theme** | Greybird GTK + Adwaita icons | `RiOS-Dark` / `RiOS-Light` GTK3/XFWM + `WhiteSur` / `MacTahoe` icons | High-fidelity dark/light aesthetic matching native RiOS design |
...
| **System Installer** | Debian Installer (curses/ncurses d-i) | Calamares Graphical Installer with RiOS branding | Modern, intuitive graphical installation with automated disk partitioning |
| **Boot Splash** | Text or Debian swirl Plymouth theme | Custom RiOS dark boot splash with centered emblem | Seamless branded startup and shutdown sequence |
...
To update RiOS to a new Debian point release or newer release (e.g. Trixie):
1. Update `--distribution` in `rios-live/auto/config`.
