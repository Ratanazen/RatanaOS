# 🏛️ RiOS Debian Architecture & Integration Report (v3.0)

This document serves as the architectural reference and audit report for integrating **Debian GNU/Linux 13 "Trixie" (x86_64 / amd64)** into **RiOS**.

---

## 1. Current RiOS Subsystem Architecture
...
- **ELF & ABI**: ELF64 parser and loader (`elf.c`) supporting 64-bit x86-64 executables (ET_EXEC, ET_DYN PT_LOAD, PT_INTERP dynamic loader linking `ld-linux-x86-64.so.2`). Dual Syscall ABI: (1) Native RiOS `int $0x80` gate; (2) x86-64 `SYSCALL`/`SYSRET` MSR gate (`LSTAR` MSR `0xC0000102`, `STAR`, `FMASK`, `KernelGSBase`).
...
1. **Track A — Dual Boot**: GRUB 2 configuration (`iso/boot/grub/grub.cfg`) featuring entries for RiOS 64-bit, Safe Text Mode, host Debian partition chainloading (`DEBIAN_ROOT` / `DEBIAN_UUID`), and `make iso-with-debian DEBIAN_ISO=...` embedded installer ISO support.
...
- **UEFI Booting**: Native UEFI booting of RiOS kernel is not implemented (requires Multiboot 1 / BIOS).
...
LEVEL 6: glibc-compatible Debian application     [PASSED]
LEVEL 7: Debian shell (/bin/sh & coreutils)      [PASSED]
LEVEL 8: Meaningful Debian userspace             [PASSED]
LEVEL 9: Advanced Debian application compat      [ACTIVE TARGET]
```
