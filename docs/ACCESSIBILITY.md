# ♿ RiOS Accessibility Support (Phase 11)

## Accessibility Verification Checklist

| # | Requirement | Test Method | Status | Notes |
|---|:---|:---|:---:|:---|
| 1 | **Orca Screen Reader** | Launch via Settings > Accessibility or \`orca\` in terminal. Verify it reads XFCE panel items, Thunar file names, and dialog buttons through the RiOS GTK theme. | PASSED | AT-SPI2 bridge (\`at-spi2-core\`) ensures widget labels are exposed. Custom GTK CSS does not override widget \`accessible-name\` properties. |
| 2 | **High-Contrast Theme** | Open XFCE Settings > Appearance > Style, select "HighContrast" or "Adwaita" as an alternative to RiOS-Dark. | PASSED | Adwaita and GNOME accessibility themes are included alongside RiOS themes. |
| 3 | **UI Scaling / Text Size** | Open XFCE Settings > Appearance > Fonts, adjust DPI scaling or font size. Verify panel, dock, and window titles scale proportionally. | PASSED | XFCE's Xft DPI setting and font size controls work correctly with RiOS theme. |
...
1. **RiOS GTK Theme Compatibility**: The custom \`RiOS-Dark\` and
   \`RiOS-Light\` GTK3 themes use standard CSS properties and do not
