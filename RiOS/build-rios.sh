#!/bin/bash
# ============================================================================
# build-rios.sh — RiOS Distribution Build System
# ============================================================================
# Transforms a Debian base system into a fully branded RiOS distribution.
# Usage: ./build-rios.sh <command>
...
RIOS_ROOTFS="${WORKSPACE}/rios-rootfs"
...
ISO_OUTPUT="${RELEASES_DIR}/RiOS-${RIOS_VERSION}-${RIOS_ARCH}"
...
cmd_audit() {
    ...
    if [[ ! -d "${DEBIAN_SOURCE}" ]]; then
        log_warn "Debian source not yet bootstrapped. Run: sudo ./build-rios.sh prepare"
        return 1
    fi

    local audit_file="${DOCS_DIR}/RIOS_DEBIAN_AUDIT.md"
    log_info "Generating audit report → ${audit_file}"

    cat > "${audit_file}" << 'AUDIT_HEADER'
# RiOS Debian Source Audit
...
AUDIT_HEADER
    ...
    # hostname
    echo "| hostname | debian | /etc/hostname | ❌ Replace | ✅ 'rios' | Pending |" >> "${audit_file}"
...
# ── PHASE 1: ROOTFS (Transform Debian → RiOS) ─────────────────────────────
cmd_rootfs() {
    log_step "Phase 1: Building RiOS Root Filesystem"
    check_root

    if [[ ! -d "${DEBIAN_SOURCE}/bin" ]]; then
        log_error "Debian source not found. Run: sudo ./build-rios.sh prepare"
        return 1
    fi

    # Copy debian-source → rios-rootfs
    if [[ ! -d "${RIOS_ROOTFS}/bin" ]]; then
        log_info "Copying Debian source → RiOS rootfs..."
        cp -a "${DEBIAN_SOURCE}" "${RIOS_ROOTFS}"
        log_ok "Rootfs copy complete"
    else
        log_info "RiOS rootfs already exists. Applying transformations..."
    fi

    local R="${RIOS_ROOTFS}"

    # ── os-release ──
    log_info "Applying RiOS identity..."
    cat > "${R}/etc/os-release" << EOF
NAME="RiOS"
PRETTY_NAME="RiOS ${RIOS_VERSION} (${RIOS_CODENAME})"
ID=rios
ID_LIKE=debian
VERSION="${RIOS_VERSION}"
VERSION_ID="${RIOS_VERSION}"
VERSION_CODENAME="${RIOS_CODENAME}"
HOME_URL="https://rios.org"
BUG_REPORT_URL="https://github.com/rios/rios/issues"
SUPPORT_URL="https://rios.org/support"
BUILD_ID="$(date +%Y%m%d)"
VARIANT="Desktop"
VARIANT_ID=desktop
LOGO=rios-logo
...
DISTRIB_ID=RiOS
DISTRIB_RELEASE=${RIOS_VERSION}
DISTRIB_CODENAME=${RIOS_CODENAME}
DISTRIB_DESCRIPTION="RiOS ${RIOS_VERSION} (${RIOS_CODENAME})"
EOF

    # ── hostname ──
    echo "rios" > "${R}/etc/hostname"
...
RiOS ${RIOS_VERSION} (${RIOS_CODENAME})
EOF

    # ── MOTD ──
    cat > "${R}/etc/motd" << EOF

  Welcome to RiOS ${RIOS_VERSION} (${RIOS_CODENAME})
  Kernel: \$(uname -r)  •  Arch: \$(uname -m)

  System tools:    rios-info, rios-version, rios-hardware
  Package manager: ratapkg install|remove|update|search|list
  Documentation:   https://rios.org/docs

EOF

    # ── GRUB ──
    if [[ -f "${R}/etc/default/grub" ]]; then
        sed -i 's/^GRUB_DISTRIBUTOR=.*/GRUB_DISTRIBUTOR="RiOS"/' "${R}/etc/default/grub"
        sed -i 's/^GRUB_TIMEOUT=.*/GRUB_TIMEOUT=5/' "${R}/etc/default/grub"
    else
        cat > "${R}/etc/default/grub" << 'EOF'
GRUB_DEFAULT=0
GRUB_TIMEOUT=5
GRUB_DISTRIBUTOR="RiOS"
...
EOF
    ...
    if [[ -d "${R}/etc/sudoers.d" ]]; then
        echo "%sudo ALL=(ALL:ALL) ALL" > "${R}/etc/sudoers.d/rios"
        chmod 440 "${R}/etc/sudoers.d/rios"
    fi

    # ── Shell profile ──
    cat > "${R}/etc/profile.d/rios.sh" << 'PROFILE'
# RiOS Shell Environment
export RIOS_VERSION="1.0.0"
export RIOS_CODENAME="Angkor"

# Custom prompt
if [ "$BASH" ]; then
    if [ "$(id -u)" -eq 0 ]; then
        PS1='\[\e[1;31m\]rios\[\e[0m\]:\[\e[1;34m\]\w\[\e[0m\]# '
    else
        PS1='\[\e[1;36m\]rios\[\e[0m\]:\[\e[1;34m\]\w\[\e[0m\]\$ '
    fi
fi
...
alias sysinfo='rios-info'
alias update='ratapkg update && ratapkg upgrade'
PROFILE
    chmod 644 "${R}/etc/profile.d/rios.sh"

    log_ok "RiOS rootfs identity applied"

    # ── Install RiOS tools ──
    log_info "Installing RiOS system tools..."
    install -m 755 "${TOOLS_DIR}/ratapkg" "${R}/usr/local/bin/ratapkg" 2>/dev/null || true
    install -m 755 "${TOOLS_DIR}/rios-info" "${R}/usr/local/bin/rios-info" 2>/dev/null || true
    install -m 755 "${TOOLS_DIR}/rios-version" "${R}/usr/local/bin/rios-version" 2>/dev/null || true
    install -m 755 "${TOOLS_DIR}/rios-update" "${R}/usr/local/bin/rios-update" 2>/dev/null || true
    install -m 755 "${TOOLS_DIR}/rios-hardware" "${R}/usr/local/bin/rios-hardware" 2>/dev/null || true
    install -m 755 "${TOOLS_DIR}/rios-network" "${R}/usr/local/bin/rios-network" 2>/dev/null || true
    install -m 755 "${TOOLS_DIR}/rios-services" "${R}/usr/local/bin/rios-services" 2>/dev/null || true
    install -m 755 "${TOOLS_DIR}/rios-config" "${R}/usr/local/bin/rios-config" 2>/dev/null || true
    install -m 755 "${TOOLS_DIR}/rios-about" "${R}/usr/local/bin/rios-about" 2>/dev/null || true

    # ── Install services ──
    log_info "Installing RiOS services..."
    for svc in "${SERVICES_DIR}"/*.service; do
        [[ -f "$svc" ]] && install -m 644 "$svc" "${R}/etc/systemd/system/" 2>/dev/null || true
    done

    log_ok "RiOS rootfs build complete"
    log_info "Rootfs size: $(du -sh ${R} | cut -f1)"
}

# ── PHASE 2: KERNEL ───────────────────────────────────────────────────────────
cmd_kernel() {
    log_step "Phase 2: RiOS Kernel Configuration"

    # For now, we use the Debian-packaged kernel.
    # Future: custom kernel build with RiOS branding.
    if [[ -d "${RIOS_ROOTFS}/lib/modules" ]]; then
        local kver="$(ls ${RIOS_ROOTFS}/lib/modules/ | head -1)"
    ...
    if [[ ! -f "${CONFIG_DIR}/rios-kernel.config" ]]; then
        cat > "${CONFIG_DIR}/rios-kernel.config" << 'EOF'
# RiOS Kernel Configuration (based on Debian defaults)
...
#   2. cp config/rios-kernel.config .config
#   3. make olddefconfig
#   4. make -j$(nproc) bindeb-pkg LOCALVERSION=-rios
#   5. Install resulting .deb packages into rios-rootfs
EOF
        log_ok "Kernel config template created: ${CONFIG_DIR}/rios-kernel.config"
    fi
}

# ── PHASE 3: PACKAGES ─────────────────────────────────────────────────────────
cmd_packages() {
    log_step "Phase 3: Installing Packages into RiOS Rootfs"
    check_root

    local R="${RIOS_ROOTFS}"
    if [[ ! -d "${R}/bin" ]]; then
        log_error "Rootfs not found. Run: sudo ./build-rios.sh rootfs"
        return 1
    fi
...
# ── PHASE 4: DESKTOP ──────────────────────────────────────────────────────────
cmd_desktop() {
    log_step "Phase 4: Installing RiOS Desktop Environment"
    check_root
    ...
    # Configure LightDM for RiOS
    log_info "Configuring RiOS login screen..."
    mkdir -p "${R}/etc/lightdm"
    cat > "${R}/etc/lightdm/lightdm-gtk-greeter.conf" << 'EOF'
[greeter]
background=/usr/share/backgrounds/rios-wallpaper.png
...
EOF
    ...
    # Create default user
    log_info "Creating default user 'rios'..."
    chroot "${R}" useradd -m -G sudo,audio,video,plugdev,netdev -s /bin/bash rios 2>/dev/null || true
    echo "rios:rios" | chroot "${R}" chpasswd 2>/dev/null || true
    echo "root:rios" | chroot "${R}" chpasswd 2>/dev/null || true

    # Generate wallpaper placeholder
    log_info "Installing RiOS branding assets..."
    mkdir -p "${R}/usr/share/backgrounds"
    # Create a simple SVG wallpaper
    cat > "${R}/usr/share/backgrounds/rios-wallpaper.svg" << 'SVGEOF'
<svg xmlns="http://www.w3.org/2000/svg" width="1920" height="1080">
  ...
  <text x="960" y="500" text-anchor="middle" font-family="sans-serif" font-size="72" font-weight="bold" fill="#e2e8f0" opacity="0.8">RiOS</text>
...
SVGEOF
    ...
    log_ok "RiOS desktop environment installed"
}

# ── PHASE 5: INSTALLER ────────────────────────────────────────────────────────
cmd_installer() {
    log_step "Phase 5: Creating RiOS Installer"
    log_info "Installer script installed at ${TOOLS_DIR}/rios-installer"
    log_ok "Installer ready (script-based, runs in live environment)"
}

# ── PHASE 6: ISO ──────────────────────────────────────────────────────────────
cmd_iso() {
    log_step "Phase 6: Building RiOS Live ISO"
    check_root
...
menuentry "RiOS Live (Safe Mode)" {
    linux /boot/vmlinuz boot=live live-media-path=/live nomodeset
    initrd /boot/initrd.img
}

menuentry "RiOS Install" {
    linux /boot/vmlinuz boot=live live-media-path=/live quiet splash rios.installer=1
    initrd /boot/initrd.img
}
...
GRUBEOF
    ...
    local iso_file="${ISO_OUTPUT}/RiOS-Live-${RIOS_ARCH}.iso"
    grub-mkrescue -o "${iso_file}" "${ISO_DIR}" \
        --product-name="RiOS" \
        --product-version="${RIOS_VERSION}" \
        2>/dev/null || \
    xorriso -as mkisofs \
        -iso-level 3 \
    ...
    if [[ -f "${iso_file}" ]]; then
        ...
        # Generate checksums
        cd "${ISO_OUTPUT}"
        sha256sum "RiOS-Live-${RIOS_ARCH}.iso" > SHA256SUMS
        log_ok "SHA256SUMS generated"

        # Generate BUILD_INFO.txt
        cat > "${ISO_OUTPUT}/BUILD_INFO.txt" << BUILDEOF
RIOS BUILD INFORMATION
==========================
RiOS Version: ${RIOS_VERSION}
Codename: ${RIOS_CODENAME}
Architecture: ${RIOS_ARCH}
...
# ── TESTING ───────────────────────────────────────────────────────────────────
cmd_test() {
    log_step "Testing RiOS"

    local iso_file="${ISO_OUTPUT}/RiOS-Live-${RIOS_ARCH}.iso"
    if [[ ! -f "${iso_file}" ]]; then
        log_error "ISO not found at ${iso_file}. Build first with: sudo ./build-rios.sh iso"
        return 1
    fi
    ...
    log_ok "QEMU launched. Check the window for RiOS boot."
}
...
cmd_clean() {
    ...
    log_info "Note: debian-source and rios-rootfs are preserved. Remove manually if needed."
}

# ── ALL ───────────────────────────────────────────────────────────────────────
cmd_all() {
    log_step "RiOS Full Build Pipeline"
    echo ""
    echo "  ██████╗  █████╗ ████████╗ █████╗ ███╗   ██╗ █████╗  ██████╗ ███████╗"
    echo "  ██╔══██╗██╔══██╗╚══██╔══╝██╔══██╗████╗  ██║██╔══██╗██╔═══██╗██╔════╝"
    echo "  ██████╔╝███████║   ██║   ███████║██╔██╗ ██║███████║██║   ██║███████╗"
    ...
    echo "  ╚═╝  ╚═╝╚═╝  ╚═╝   ╚═╝   ╚═╝  ╚═╝╚═╝  ╚═══╝╚═╝  ╚═╝ ╚═════╝ ╚══════╝"
    echo ""
    echo "  Version ${RIOS_VERSION} (${RIOS_CODENAME}) — Build System"
    echo ""
    ...
    log_ok "RiOS ${RIOS_VERSION} build finished successfully!"
    log_info "ISO: ${ISO_OUTPUT}/RiOS-Live-${RIOS_ARCH}.iso"
    log_info "Run 'sudo ./build-rios.sh test' to test in QEMU"
}
...
case "${1:-help}" in
    ...
    *)
        ...
        echo "  rootfs     Transform rootfs into RiOS"
