#!/usr/bin/env bash
# ==============================================================================
# RatanaOS Installer: User & Authentication Configuration Module
# ==============================================================================
set -euo pipefail

configure_user() {
    local target_mount="${1:-/mnt/target}"
    local username="${2:-ratana}"
    local password="${3:-ratana}"
    local fullname="${4:-Ratana}"
...
if [ "${BASH_SOURCE[0]}" = "${0}" ]; then
    configure_user "${1:-/mnt/target}" "${2:-ratana}" "${3:-ratana}" "${4:-Ratana}" "${5:-ratana}"
fi
